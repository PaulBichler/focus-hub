...
alertHtmlElements.set('info', { color: 'blue', alertTime: -1 });
...
export function addAlert(alert) {
    ...
    `<div name="alert-` + alertMessageHash + `" class="flex p-4 mb-1 text-` + alertSettings.color + `-800 border-t-4 border-` + alertSettings.color + `-300 bg-` + alertSettings.color + `-50 dark:text-` + alertSettings.color + `-400 dark:bg-gray-800 dark:border-` + alertSettings.color + `-800" role="alert">
    ...
    const targetEl = toastContainer.firstChild;
    const options = {
